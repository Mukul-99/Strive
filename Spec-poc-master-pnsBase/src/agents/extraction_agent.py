...
def process_pns_data(state: SpecExtractionState) -> SpecExtractionState:
    """Process PNS JSON data as a regular agent"""
    ...
    try:
        logger.info("Starting PNS data processing as agent")
        
        # Process PNS JSON
        pns_result = process_pns_json(state["pns_json_content"])
        
        if pns_result["status"] == "completed":
            # Convert PNS result to agent format
            extracted_specs = pns_result["extracted_specs"]
            
            if extracted_specs:
                # Format as table similar to other agents
                formatted_specs = "# PNS SPECIFICATIONS (Top 5 by frequency)\n"
                formatted_specs += "Rank,Specification,Options,Frequency,Status,Priority\n"
                
                for i, spec in enumerate(extracted_specs, 1):
                    formatted_specs += f"{i},{spec.get('spec_name', 'N/A')},{spec.get('option', 'N/A')},{spec.get('frequency', 'N/A')},{spec.get('spec_status', 'N/A')},{spec.get('importance_level', 'N/A')}\n"
                
                processing_time = time.time() - start_time
                
                result = {
                    "source_type": "pns-json",
                    "raw_data_count": len(extracted_specs),
                ...
                logger.info(f"PNS agent completed successfully with {len(extracted_specs)} specs")
            else:
                result = {
                    "source_type": "pns-json",
                    "raw_data_count": 0,
                    ...
                    "processing_time": round(time.time() - start_time, 2),
                    "status": "failed",
                    "error": "No specifications extracted from PNS data",
                    "chunks_processed": 0
                }
        else:
            result = {
                ...
                "processing_time": round(time.time() - start_time, 2),
                "status": "failed",
                "error": pns_result.get("error", "PNS processing failed"),
                "chunks_processed": 0
            }
            
    except Exception as e:
        error_msg = str(e)
        logger.error(f"Error processing PNS data as agent: {error_msg}")
    ...
    # Return updates to unique keys only
    return {
        "pns_data_status": result["status"],
        "pns_data_result": result,
        "pns_data_error": result.get("error", ""),
        "logs": [f"Agent pns_data: {_get_status_message(result)} in {result['processing_time']:.2f}s"]
    } 