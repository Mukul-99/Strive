...
def run_three_stage_workflow_blocking(product_name: str, uploaded_files: dict, pns_json_content: str = ""):
    """Run the 3-stage workflow: CSV agents → triangulation → PNS processing → final triangulation"""
    try:
        ...
        status_text.text("🚀 Starting 3-stage analysis...")
        progress_bar.progress(5)
        
        logger.info(f"Starting 3-stage workflow for product: {product_name}")
        
        # Stage 1: Run CSV agents and triangulation (0-60%)
        from src.agents.workflow import run_spec_extraction
        csv_result_state = run_spec_extraction(initial_state)
        
        progress_bar.progress(60)
        status_text.text("📊 CSV triangulation completed...")
        
        # Stage 2: Process PNS JSON if available (60-80%)
        if pns_json_content:
            from src.agents.pns_processor import process_pns_json
            pns_result = process_pns_json(pns_json_content)
            
            # Update state with PNS results
            csv_result_state.update({
                "pns_processing_status": pns_result["status"],
                "pns_extracted_specs": pns_result["extracted_specs"],
                "pns_error": pns_result["error"]
            })
        
        progress_bar.progress(80)
        status_text.text("📋 PNS processing completed...")
        
        # Stage 3: Final consensus - only common specs between CSV and PNS results (80-100%)
        if csv_result_state.get("triangulated_result") or csv_result_state.get("pns_extracted_specs"):
            from src.agents.triangulation_agent import final_triangulate_results
            final_result_state = final_triangulate_results(csv_result_state)
            
            # Merge final results
            csv_result_state.update(final_result_state)
        
        # Update progress to complete
        progress_bar.progress(100)
        status_text.text("✅ 3-stage analysis completed!")
        
        # Store results
        st.session_state.processing_active = False
        st.session_state.final_results = csv_result_state
        
        st.success("✅ 3-stage analysis completed successfully!")
        st.rerun()
        
    except Exception as e:
        logger.error(f"3-stage workflow execution failed: {str(e)}")
        st.error(f"❌ Workflow failed: {str(e)}")
        st.session_state.processing_active = False
...
def render_input_view():
    ...
    if uploaded_files:  # At least one CSV file is required
        ...
        if start_processing:
            ...
            # Start 3-stage processing with blocking mode
            st.info("🚀 Starting 3-stage analysis...")
            run_three_stage_workflow_blocking(product_name, uploaded_files, pns_json_content)
...
def render_results_view():
    ...
    if st.session_state.restart_triangulation:
        ...
        if uploaded_files:
            ...
            st.info("🔄 Restarting 3-stage analysis...")
            run_three_stage_workflow_blocking(product_name, uploaded_files, pns_json_content)
