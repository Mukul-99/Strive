...
def render_single_stage_results(final_results: Dict[str, Any]):
    """Render single-stage results: All agents processed to find common specifications"""
    
    st.markdown("## 📊 Single-Stage Analysis Results")
    st.markdown("*All datasets (CSV + PNS JSON) processed by 5 parallel agents to identify common specifications*")
    ...
    # Display common specifications results
    st.markdown("### 🎯 Common Specifications")
    
    if triangulated_table:
        # Check if this is the "No common specifications" case
        if len(triangulated_table) == 1 and triangulated_table[0].get('Specification') == 'No Common Specifications':
            st.warning("⚠️ No common specifications found across sources")
        else:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Common Options": st.column_config.TextColumn("Common Options", width="large"),
                    "Sources Found In": st.column_config.TextColumn("Sources Found In", width="medium", help="Shows which datasets mentioned this specification")
                }
            )
    elif triangulated_result:
        st.markdown("#### 🎯 Common Specifications Result")
        st.markdown(triangulated_result)
    else:
        st.info("No common specifications results available")
...
def download_single_stage_results(final_results: Dict[str, Any]):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Sheet 1: Common Specifications Results
            triangulated_table = final_results.get("triangulated_table", [])
            if triangulated_table:
                df_triangulated = pd.DataFrame(triangulated_table)
                df_triangulated.to_excel(writer, sheet_name='Common_Specifications', index=False)
        ...
        st.download_button(
            label="📥 Download Common Specifications (Excel)",
            data=output.getvalue(),
            file_name="common_specifications_results.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
