...
class TriangulationAgent:
    ...
    def _build_triangulation_prompt(self, product_name: str, datasets: List[Dict], all_dataset_outputs: Dict) -> str:
        """Build simple common specifications extraction prompt"""
        ...
        # Simple common specifications extraction prompt
        prompt = f"""<role>
You are a data analyst specializing in finding common specifications across multiple data sources for {product_name}.
</role>

<task>
Find specifications that appear in 2 or more sources and identify their common options. Focus only on extracting what's actually common across sources.
</task>

<extraction_rules>
1. IDENTIFY COMMON SPECIFICATIONS:
   • Find specifications that appear in 2+ sources (semantic matching allowed)
...
2. FIND COMMON OPTIONS:
   • For each common specification, identify options that appear in 2+ of the sources that contain that specification
   • Only include options that are actually present across multiple sources
   • Use exact option names from the data

3. EXCLUDE:
   • Specifications appearing in only 1 source
   • Options appearing in only 1 source
   • Generic terms without specific options

4. RANKING:
   • Primary: Number of sources (4 sources > 3 sources > 2 sources)
   • Secondary: Frequency within same source count
</extraction_rules>
...
<output_requirements>
Create a simple table with EXACTLY this format:

| Specification | Common Options | Sources Found In |

Requirements:
1. Specification: Use clear, professional name (prefer the most common term used)
2. Common Options: List only options that appear in 2+ sources for this spec (comma-separated)
3. Sources Found In: Format as "X/{len(datasets)} sources: source1, source2, source3"

CRITICAL INSTRUCTIONS:
• Only include specifications found in 2+ sources
• Only include options found in 2+ sources for that specification
• Order by source count (descending), then by frequency
• If no common specifications found, respond with "No common specifications found across sources"
• If a specification exists in multiple sources but has no common options, exclude it
</output_requirements>

<example_output>
| Power Rating | 5 KVA, 10 KVA, 15 KVA | 4/{len(datasets)} sources: search_keywords, whatsapp_specs, rejection_comments, lms_chats |
| Material | Steel, Aluminium | 3/{len(datasets)} sources: whatsapp_specs, rejection_comments, lms_chats |
| Phase | Single Phase, Three Phase | 2/{len(datasets)} sources: search_keywords, whatsapp_specs |
</example_output>
"""
    ...
    def _parse_triangulation_result(self, result: str) -> List[Dict[str, Any]]:
        """Parse common specifications result into structured table format for export"""
        try:
            ...
            # Check for "No common specifications found" message
            if "No common specifications found" in result:
                logger.info("No common specifications found in the result")
                return [{
                    'Rank': 1,
                    'Specification': 'No Common Specifications',
                    'Common Options': 'No common specifications found across sources',
                    'Sources Found In': 'N/A'
                }]
            ...
            for i, line in enumerate(lines):
                ...
                if not line:
                    continue
                if 'Specification' in line and 'Common Options' in line:
                    continue
                if line.startswith('|--') or line.startswith('|-'):
                    continue
                if line.count('|') < 2:  # Need at least 3 columns
                    continue
                ...
                if '|' in line:
                    ...
                    # Ensure we have exactly 3 parts (spec, options, sources)
                    if len(parts) >= 3:
                        table_data.append({
                            'Rank': rank,
                            'Specification': parts[0],
                            'Common Options': parts[1],
                            'Sources Found In': parts[2].strip()
                        })
                        rank += 1
                        logger.info(f"Successfully added row {rank-1}: {parts[0]} with sources: {parts[2].strip()}")
            
            # Debug log
            logger.info(f"Successfully parsed {len(table_data)} table rows")
            
            # Sort by dataset count for prioritization (extract from "Sources Found In" column)
            if table_data:
                table_data_with_counts = []
                for item in table_data:
                    dataset_count = self._extract_dataset_count_from_sources(item.get('Sources Found In', 'N/A'))
            ...
            logger.error(f"Error parsing common specifications result: {e}")
            # Return a fallback structure for the new format
            return [{
                'Rank': 1,
                'Specification': 'Parse Error',
                'Common Options': 'Could not parse result',
                'Sources Found In': 'N/A'
            }]
    
    def _extract_dataset_count_from_sources(self, sources_column: str) -> int:
        """Extract dataset count from Sources Found In column format like '4/5 sources: source1, source2, source3'"""
        try:
            if not sources_column or sources_column == 'N/A':
                return 0
            
            # Extract the number before the "/" (e.g., "4" from "4/5 sources: ...")
            if '/' in sources_column and 'sources:' in sources_column:
                ...
                # Fallback: count commas in source list + 1
                if 'sources:' in sources_column:
                    source_list = sources_column.split('sources:')[1].strip()
                    dataset_count = len([s.strip() for s in source_list.split(',') if s.strip()])
                    logger.debug(f"Counted {dataset_count} sources from list: '{source_list}'")
                    return dataset_count
                else:
                    logger.debug(f"No standard format found in sources '{sources_column}', assuming 1 dataset")
                    return 1
